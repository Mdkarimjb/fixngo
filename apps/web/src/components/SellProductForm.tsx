...
export function SellProductForm() {
  ...
  const mutation = useMutation({
    mutationFn: async (payload: NewListing) =>
  ...
  return (
    <form onSubmit={submit} className="dashboard-panel p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="dashboard-eyebrow">Resell & reuse</p><h2 className="dashboard-title">Sell an old product</h2><p className="mt-1 text-sm text-slate-500">List a usable home product in minutes.</p></div><span className="hidden h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-violet-50 text-violet-600 sm:flex"><Tag className="h-5 w-5" /></span></div>

      <label htmlFor="listing-title" className="mt-6 block text-sm font-bold text-slate-700">
        Title
      </label>
      <input
        id="listing-title"
      ...
      <label htmlFor="listing-description" className="mt-4 block text-sm font-bold text-slate-700">
      ...
      <label htmlFor="listing-price" className="mt-4 block text-sm font-bold text-slate-700">
        Price (₹)
      </label>
      <input
        id="listing-price"
      ...
      >
        {mutation.isPending ? 'Listing…' : <>List for sale <ArrowRight className="h-4 w-4" /></>}
      </button>

      {mutation.isSuccess && (
        <p className="mt-2 flex items-center gap-1.5 text-sm text-green-600">
          <CheckCircle2 className="h-4 w-4" /> Product listed!
        </p>
      )}
      {mutation.isError && (
