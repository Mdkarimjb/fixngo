...
export function CustomerHome() {
  ...
  const rateJob = useMutation({
    mutationFn: async ({ jobId, rating }: { jobId: string; rating: number }) =>
      (await api.patch<Job>(`/jobs/${jobId}/rate`, { rating })).data,
    onSuccess: (updatedJob) => {
      queryClient.setQueryData<Job[]>(["customer", "jobs"], (current = []) =>
  ...
  return (
    <RoleLayout
      title="Welcome back, Vijay"
      subtitle="Book, track and manage every home service from one place."
    >
      <div id="overview" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          ...
          value={listingList.length}
          hint="Your active items"
          tone="violet"
        />
        <Metric
          icon={WalletCards}
      ...
      <div className="mt-6 grid items-start gap-6 xl:grid-cols-[1.45fr_.75fr]">
        <div className="space-y-6">
          ...
          <section id="bookings" className="dashboard-panel p-5 sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="dashboard-eyebrow">Bookings</p>
                <h2 className="dashboard-title">Your service activity</h2>
              </div>
              <button
                type="button"
            ...
            {jobsLoading ? (
              ...
              <ul className="mt-5 grid gap-4 2xl:grid-cols-2">
                {jobList.map((job) => (
                  <JobTicket
                    key={job.id}
                    job={job}
                    onRate={(rating) =>
                      rateJob.mutate({ jobId: job.id, rating })
                    }
                    ratingPending={
                      rateJob.isPending && rateJob.variables?.jobId === job.id
                    }
                  />
                ))}
              </ul>
              ...
              <EmptyState
                ...
                text="Choose a service above and your booking will appear here with live tracking."
                action="Book your first service"
              />
            )}
          </section>
        </div>
        ...
        <aside className="space-y-6">
          <section className="overflow-hidden rounded-2xl bg-primary text-white shadow-lg shadow-blue-900/10">
            ...
            <div className="p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-blue-100">
                Professional cleaning from ₹399 with verified experts and safe
                products.
              </p>
              <a
                href="#new-service"
                className="mt-5 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-sm font-bold text-white"
              >
          ...
          <section className="dashboard-panel p-5">
            <p className="dashboard-eyebrow">Your benefits</p>
            <h2 className="dashboard-title">FixNGo protection</h2>
            <div className="mt-5 space-y-4">
              {[
                [
                  "30-day service warranty",
                  "We return and resolve covered issues.",
                ],
                [
                  "Verified professionals",
                  "Every expert is trained and background-checked.",
                ],
                ["Pay after service", "UPI, cards and cash accepted."],
              ].map(([title, text]) => (
      ...
      <section
        id="marketplace"
        className="mt-6 grid items-start gap-6 xl:grid-cols-[.8fr_1.2fr]"
      >
        <SellProductForm />
        <div className="dashboard-panel p-5 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="dashboard-eyebrow">Marketplace</p>
              <h2 className="dashboard-title">My listings</h2>
            </div>
            <span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-700">
              {listingList.length} items
            </span>
          </div>
          {listingsLoading ? (
            <SkeletonList />
              ...
              {listingList.map((item) => (
                <li key={item.id} className="flex items-center gap-4 py-4">
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`text-[10px] font-bold uppercase ${item.sold ? "text-slate-400" : "text-emerald-600"}`}
                    >
            ...
            <EmptyState
              ...
              text="Give an old appliance or home product a second life."
...
function EmptyState({
  ...
  return (
    <div className="mt-5 flex flex-col items-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-6 py-10 text-center">
