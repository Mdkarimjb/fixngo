...
export function Login({ modal = false, onClose }: { modal?: boolean; onClose?: () => void }) {
  ...
  const loginCard = (
      <div ref={dialogRef} role={modal ? 'dialog' : undefined} aria-modal={modal || undefined} aria-labelledby="login-title" tabIndex={modal ? -1 : undefined} className="relative w-full max-w-sm rounded-3xl bg-white p-8 shadow-xl ring-1 ring-gray-100 outline-none">
        {modal && (
          <button type="button" onClick={onClose} aria-label="Close sign in" className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full text-gray-500 transition hover:bg-gray-100 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
            <X className="h-5 w-5" />
          </button>
        )}
        ...
        <h1 id="login-title" className="mt-6 font-heading text-xl font-bold text-gray-900">
  ...
  if (modal) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 px-4 py-8 backdrop-blur-sm" onMouseDown={(event) => {
